...
class AsyncCallbackListener(AsyncListener):
    ...
    def __init__(self, callback: Callable[[Any], Awaitable], max_queue_size: int = 10):
        super().__init__(max_queue_size)
        self._callback = callback
        # 백그라운드 루프에서 run() 코루틴 자동 실행
        asyncio.run_coroutine_threadsafe(self.run(), AsyncBroker()._loop)
...
class AsyncBroker:
    ...
    def subscribe(
        ...
        asyncio.run_coroutine_threadsafe(
            self._subscribe_coro(event, listener, max_queue_size), self._loop
        )

    async def _subscribe_coro(
        self,
        event: str,
        listener: Callable[[Any], Awaitable] | AsyncListener,
        max_queue_size: int,
        ...
        async with self._lock:
            if not isinstance(listener, AsyncListener):
                # 함수·메서드·코루틴함수 → 래핑
                if (
                    inspect.isfunction(listener)
                    or inspect.ismethod(listener)
                    or inspect.iscoroutinefunction(listener)
                ):
                    listener = AsyncCallbackListener(listener, max_queue_size)
                else:
                    raise ValueError(
    ...
    def emit(self, message: AsyncMessageType):
        ...
        ■ 동기 메서드
        """
        asyncio.run_coroutine_threadsafe(self._emit_coro(message), self._loop)
